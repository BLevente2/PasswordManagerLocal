...
public sealed class Phase3ApplicationPreferencesArchitectureTests
{
    ...
    public void PreferencesInfrastructureReferencesOnlyContractsProject()
    {
        var project = XDocument.Load(Path.Combine(
            GetRepositoryRoot(),
            "PasswordManagerLocal.Preferences",
            "PasswordManagerLocal.Preferences.csproj"));
        var references = project.Descendants("ProjectReference")
            .Select(reference => reference.Attribute("Include")?.Value ?? string.Empty)
            .ToArray();

        CollectionAssert.AreEqual(
            new[] { @"..\PasswordManagerLocal.Contracts\PasswordManagerLocal.Contracts.csproj" },
            references);
    }
    ...
    public void ContractsPreferenceBoundaryRemainsPlatformNeutral()
    {
        ...
        var preferenceSources = Directory.EnumerateFiles(
                Path.Combine(root, "PasswordManagerLocal.Contracts", "Preferences"),
                "*.cs",
    ...
    public void DefaultsAreNotRedefinedByApplicationConsumers()
    {
        ...
        {
            Path.Combine(root, "PasswordManagerLocal", "PasswordManagerLocal.Frontend"),
            Path.Combine(root, "PasswordManagerLocal.Windows.Agent"),
            Path.Combine(root, "PasswordManagerLocal", "PasswordManagerLocal.Android")
        };
    ...
    {
        var constructor = typeof(PasswordManagerLocal.Frontend.App).GetConstructor(Type.EmptyTypes);
    ...
    public void FrontendAndAgentSharePreferencesWithoutForbiddenDependencies()
    {
        ...
        var frontendReferences = ReadProjectReferences(Path.Combine(
            ...
            "PasswordManagerLocal",
            "PasswordManagerLocal.Frontend",
            "PasswordManagerLocal.Frontend.csproj"));
    ...
    {
        var agentRoot = Path.Combine(GetRepositoryRoot(), "PasswordManagerLocal.Windows.Agent");
