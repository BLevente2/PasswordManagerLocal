...
public partial class PasswordTagListView : UserControl
{
    private const string HoveredClass = "hovered";
    private const string InteractiveListItemClass = "interactiveListItem";

    private readonly MultiSelectionListItemPointerHandler<PasswordTagManagementItemViewModel> _multiSelectionPointerHandler;

    public PasswordTagListView()
    {
        InitializeComponent();
        _multiSelectionPointerHandler = new MultiSelectionListItemPointerHandler<PasswordTagManagementItemViewModel>(
            this,
            BeginMultiSelection);
    }
    ...
    private void BeginMultiSelection(PasswordTagManagementItemViewModel tag)
    {
        if (DataContext is PasswordsViewModel viewModel)
        {
            viewModel.BeginPasswordTagMultiSelection(tag);
        }
    }

    private static void ExecuteEditCommand(Border item)
    {
        if (item.DataContext is not PasswordTagManagementItemViewModel tag)
        {
            return;
        }
