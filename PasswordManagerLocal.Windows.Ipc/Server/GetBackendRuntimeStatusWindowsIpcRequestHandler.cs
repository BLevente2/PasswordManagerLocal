...
public sealed class GetBackendRuntimeStatusWindowsIpcRequestHandler : IWindowsIpcRequestHandler
{
    ...
    public GetBackendRuntimeStatusWindowsIpcRequestHandler(
    ...
    public IpcOperationId OperationId => IpcOperationId.GetBackendRuntimeStatus;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var status = await _statusProvider.GetBackendRuntimeStatusAsync(cancellationToken);
        _contractValidator.Validate(status);
        return context.Success(status, WindowsIpcJsonContext.Default.BackendRuntimeStatusDto);
    }
}
